...
trait TradeModel { this: RefModel =>
  ...
  val rates: Map[TaxFeeId, BigDecimal] =
  ...
  val taxFeeForMarket: Map[Market, List[TaxFeeId]] =
    Map(
      Other -> List(TradeTax, Commission),
      Singapore -> List(TradeTax, Commission, VAT)
    )
  ...
  private val valueAs: Trade => TaxFeeId => BigDecimal = {
    trade =>
      { tid =>
  ...
  // all tax/ees for a specific trade
  val taxFeeCalculate
      : Trade => List[TaxFeeId] => List[(TaxFeeId, BigDecimal)] = { t => tids =>
